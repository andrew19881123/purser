...
| `PURSER_AGENT_PREFIX_CACHING_FACTOR` | `0.0` | Fraction of inference tokens that hit the KV cache (0–1). Forwarded to the Control Plane via `HardwareProfile` for Planner calibration. A value of `0.0` (default) tells the Planner that no prefix reuse is expected; `1.0` means every prompt token is a cache hit. Values outside [0, 1] are clamped silently. |
