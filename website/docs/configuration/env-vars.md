...
| `PURSER_PLANNER_ORDERING_THRESHOLD` | `10` | Fleet size at or below which the planner uses the exact Held-Karp algorithm to find the minimum-cost pipeline ordering. Above this threshold the planner switches to the nearest-neighbour + 2-opt heuristic. Held-Karp has O(2^N·N²) complexity: at N=10 the DP uses ~10k states; at N=20 (~160 MB) it remains feasible; at N≥32 the 2^N table would cause an OOM crash. **Valid range: 1–20.** Values outside this range are silently rejected and the default (10) is used. Raise this value only on planners with abundant memory and CPU. Read once at startup; restart the process to apply a new value. |
