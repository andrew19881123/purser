...
Planner output:
  ...
  estimated decode throughput: 18–24 tok/s
```
...
- A Kubernetes cluster (or docker compose) for the Control Plane on one machine, OR
...
On any machine (can be a CPU-only machine or one of the GPU nodes):

```bash
docker compose up -d   # uses the demo stack for simplicity
```

Or with Helm on Kubernetes:
...
Note the Control Plane address (e.g. `http://192.168.1.10:8080`).
...
PURSER_CONTROL_PLANE_ADDR=http://192.168.1.10:8080
