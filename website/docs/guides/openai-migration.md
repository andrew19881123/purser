...
property of the gateway, and this is the list — the
[Gateway API reference](../api/gateway.md) is the authoritative description of each
one's behaviour:
...
| `POST /v1/embeddings` | Supported. Leave `stream` unset — see [Common pitfalls](#common-pitfalls) |
...
**Requests are capped at 4 MB.** The limit applies to the three POST inference
endpoints, not to `GET /v1/models`. Long conversation histories and large embedding
batches are the usual way to hit it — split the batch rather than raising
expectations. Note the cap is enforced before Purser's handler runs, so a rejection
does not come back as one of the JSON error envelopes below.
...
**Leave `stream` unset on `/v1/embeddings`.** The gateway honours `stream` on
**every** inference endpoint — it does not special-case embeddings. Since an
embedding response is not a token stream, asking for one puts the gateway into SSE
relay mode over a response the engine will never stream, and there is no guard that
rejects the combination for you. Send embedding requests without `stream`, or with
`stream: false`.
