...
Starting with v0.2.0, all Purser binary tarballs are signed with [cosign](https://docs.sigstore.dev/cosign/overview/) using keyless Sigstore signing via GitHub Actions OIDC. A cosign bundle file (`*.tar.gz.bundle`) is published alongside each tarball as a release asset — it embeds the certificate and signature so verification works offline.
...
Download the tarball and its bundle, then verify:

```bash
VERSION=0.2.0

# Download the tarball and its cosign bundle
curl -LO https://github.com/andrew19881123/purser/releases/download/v${VERSION}/purser-agent-linux-amd64-v${VERSION}.tar.gz
curl -LO https://github.com/andrew19881123/purser/releases/download/v${VERSION}/purser-agent-linux-amd64-v${VERSION}.tar.gz.bundle

# Verify the signature
cosign verify-blob \
  --bundle purser-agent-linux-amd64-v${VERSION}.tar.gz.bundle \
  --certificate-identity-regexp="https://github.com/andrew19881123/purser/.*" \
  --certificate-oidc-issuer="https://token.actions.githubusercontent.com" \
  purser-agent-linux-amd64-v${VERSION}.tar.gz
```

The same pattern applies to the gateway and control-plane tarballs — substitute `purser-gateway` or `control-plane` in the filenames. A successful verification prints `Verified OK` and exits 0.
